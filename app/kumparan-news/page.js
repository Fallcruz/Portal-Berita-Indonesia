            ...
            <div className="lg:w-[80%] w-[90%] flex flex-col lg:p-12 p-6 mx-auto gap-8">
                <div className="flex items-center justify-between">
                    ...
                    <div className="basis-[60%]">
                        <h1 className="lg:text-4xl text-2xl font-semibold text-center">Kumparan News</h1>
                    </div>
                    <div className="basis-[40px]"></div>
                </div>
                <div className="flex flex-col gap-4">
                    {dataKumparan.map((data) => (
                        <PageNewsCard key={data.title} image={data.image.large} title={data.title} link={data.link} description={data.description} date={data.isoDate} />
                    ))}
                </div>
            </div>
        </main>
